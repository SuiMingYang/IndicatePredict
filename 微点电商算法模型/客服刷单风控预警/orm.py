...
class mysql_ORM():
    ...
    def select_chan(self,connect):
        cursor = connect.cursor()
        sql = "SELECT channelid,name FROM channel"
        # 查询数据
        try:
            cursor.execute(sql)
    ...
    def add(self,connect,data):
        cursor = connect.cursor()
        sql = 'insert into userinfo_course(name,Introduct) values (%s,%s)'
        data = ((u'语文',''), (u'数学','aaa'))
...
if __name__ == "__main__":
    ...
    conn=obj.connect_fc()
    #obj.add(conn,('www', 'www', 'www', '222', 'www', 'www', 'www', 'www', 'ww-A组', 'ww', 'ww'))
